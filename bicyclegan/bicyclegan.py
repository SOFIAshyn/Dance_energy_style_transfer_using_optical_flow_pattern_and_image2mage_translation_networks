...
if opt.epoch != 0:
    ...
    encoder.load_state_dict(torch.load(f"saved_models/{opt.dataset_name}/encoder_{opt.epoch}.pth"))
    D_VAE.load_state_dict(torch.load(f"saved_models/{opt.dataset_name}/D_VAE_{opt.epoch}.pth"))
    D_LR.load_state_dict(torch.load(f"saved_models/{opt.dataset_name}/D_LR_{opt.epoch}.pth"))
else:
    # Initialize weights
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0:
        ...
        torch.save(encoder.state_dict(), f"saved_models/{opt.dataset_name}/encoder_{epoch}.pth")
        torch.save(D_VAE.state_dict(), f"saved_models/{opt.dataset_name}/D_VAE_{epoch}.pth")
        torch.save(D_LR.state_dict(), f"saved_models/{opt.dataset_name}/D_LR_{epoch}.pth")
